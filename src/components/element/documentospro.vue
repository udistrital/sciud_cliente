  ...
  <div class="col p-0 m-0 docs slide" :id="id">
                ...
                <div class="row">
                  <div class="col-9">
                    ...
                    <transition name="fade">
                      <div class="row" v-if="doc_description">
                        <div class="col">
                          <div
                            class="card-body m-0 mb-3 p-0"
                            v-html="doc_description"
                          ></div>
                        </div>
                      </div>
                    </transition>
                  ...
                  <div class="col-3">
                    <div class="form-group file-container">
                      ...
                      <label class="error slide" id="file-error"
                        >Requerido</label
                      >
                    </div>
                  </div>
              ...
              <template #tpl="{ data }" v-if="visibleButton">
                <span class="cmds">
                  <a
                    ...
                    @click.prevent="documentEdit(data.data)"
                    href="#"
                  >
    ...
    <div class="row mt-3 pb-0" v-if="is_dev && debug">
      <div class="col">
        <div class="card">
          <div class="card-body">
            <span class="font-weight-semibold">editMode:</span>
            {{ visibleButton }}
            <span class="font-weight-semibold">botonUploadVisible:</span>
            {{ JSON.stringify(botonUploadVisible, null, 3) }}
            <hr class="sep mb-0" />
            <span class="font-weight-semibold">mainObj:</span>
            {{ JSON.stringify(mainObj, null, 3) }}
            <hr class="sep mb-0" />
            <span class="font-weight-semibold">tipos:</span>
            {{ JSON.stringify(tipos, null, 3) }}
            <span class="font-weight-semibold">document_types:</span>
            {{ JSON.stringify(document_types, null, 3) }}
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  created() {
	...
	setTimeout(
		function(){
			if (
      typeof root.botonUploadVisible != "undefined" &&
      root.botonUploadVisible != null
    ) {
    ...
    if (
      typeof root.botonUploadVisible.id != "undefined" &&
      root.botonUploadVisible.id != null
    ) {
  ...
  mounted() {
    ...
    setTimeout(function () {
      ...
      root.uploader =
        typeof root.$refs.uploader !== "undefined"
          ? root.$refs.uploader.instance
          : null;
      if (root.uploader !== null)
        root.uploader.option({
          ...
          onValueChanged: (e) => {
            ...
            let errors = $(e.element).find(
              ".dx-fileuploader-file-status-message span"
            );
            if (errors.length > 0) {
          ...
          onContentReady: (e) => {
            ...
            var el = $(e.element);
            el.find(
              ".dx-fileuploader-upload-button,.dx-fileuploader-input-container"
            ).hide();
            root.btnDocSelect = el.find(
              ".dx-fileuploader-button.dx-button-has-text:first"
            );
            let b = root.btnDocSelect;
            root.btnDocSelect
              .removeAttr("class")
              .addClass(
                "btn btn-main btn-sm btn-labeled btn-labeled-left legitRipple w-100"
              );
            if (root.btnDocSelect.find("i").length <= 0)
              root.btnDocSelect.html(
                '<b><i class="icon-file-pdf"></i></b><span>' +
                  root.btnDocSelect.find(".dx-button-text").text() +
                  "</span>"
              );
          },
        });
  ...
  computed: {
    ...
    dataSource: function () {
      ...
      if (typeof root.mainObj.id === "undefined" || root.mainObj.id === null)
        return null;
      return DxStore({
        key: ["id"],
        endPoint: root.ep,
  ...
  methods: {
    ...
    documentLink: (data) => {
      ...
      let btn = $(
        `<a href="${dl}" target="_blank" class="color-main-600 font-weight-semibold"><i class="icon-link"></i> OBSERVAR</a>`
      );
    },
    documentShow: async (data) => {
      ...
      root.loaderShow(
        "Cargando Documento",
        $("#panel-documentos").length > 0 ? "#panel-documentos" : "#" + root.id
      );
      setTimeout(async function () {
        ...
        try {
          let doc = JSON.parse(
            typeof data !== "undefined" ? data.doc_path : root.current.doc_path
          );
          console.log("doc", doc);
          let blob = await root.getDoc(doc);
          root.tituloDocumento = `Documento SICIUD v2.0: ${data.doc_name} - ${
            data.document_type_name
          } (${doc.fileSize.formatSize()})`;
          console.log("blob", blob);
          // 202105101656: https://stackoverflow.com/a/24548869
          var url = URL.createObjectURL(blob);
          root.docLink =
            root.baseUrl + "view/index.html?file=" + encodeURIComponent(url);
    ...
    documentSave: async () => {
      ...
      if (result.isValid) {
        ...
        if (root.file !== null) {
          console.log("root.tipos", root.tipos);
          console.log("root.file", root.file);
          root.file["title"] = root.tipos.find(
            (o) => o.id === root.baseObj.document_type_id
          ).st_name;
    ...
    documentTypeFilter: (cb) => {
      ...
      console.log("root.item_document_types", root.item_document_types);
      if (root.item_document_types.length === root.tipos.length)
        $(root.btnDocAdd).fadeOut();
      if (root.item_document_types.length <= 0) {
        ...
        root.tipos.forEach((item) => {
          ...
          if (
            item.multiple ||
            !ignore.includes(item.id.toString()) ||
            (root.current !== null && root.current.document_type_id === item.id)
          ) {
