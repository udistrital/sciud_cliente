				...
				<div class="card">
					<div class="card-body group-detail" id="item-panel">
						<div class="col-md-12 mt-1">
							<div class="alert alert-warning" role="alert" v-if="!validDateConv">
											...
											<div class="card-body mb-0 pb-0 pt-2">
												<h2>
													<span class="item-title mr-1"
														>Por favor verifique bien la información diligenciada, esta aplicación no se podrá editar más adelante.</span
													>
												</h2>
												<div class="row">
													<!-- formulatio -->
														...
														<DxButton @click="save" v-if="validDateConv" class="nb">
															...
															<template #default>
																<span class="btn btn-main btn-labeled btn-labeled-right btn-sm legitRipple">
																	{{ mode == "edit" ? "Crear Aplicación" : "Aplicar.." }} <b><i class="icon-database-add"></i></b>
																</span>
															</template>
														</DxButton>
													</div>
...
export default {
	...
	created: async function() {
		root = this;
		root.baseObj.state_id = root.get_sub_type_id("borrador");
		console.warn("root.baseObj.state_id => ", root.baseObj.state_id);
	...
	computed: {
		...
		validDateConv() {
			...
			if (root.baseItem.call_type_name != "Movilidad") {
				this.go(0, "/convocatoria", `Cargando Convocatorias`);
			}
		...
		save() {
			...
			if (result.isValid) {
				...
				let dto = {
					...
					cb: function(item) {
						root.$info(
							"Estimado investigador, Su aplicación fue creada con éxito. <br/> Guarde en la plataforma los documentos requeridos y luego en la columna acciones presione el botón enviar."
						);
