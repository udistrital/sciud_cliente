        ...
        <div class="p-0">
          <div class="page-header header-elements-md-inline mb-2">
            <div class="page-title p-0 m-0">
              <h1>
                <i class="icon-pushpin mr-1 color-main-600"></i>
            ...
            <div class="header-elements">
              <span class="cmds">
                <button
                  v-if="permisoGuardar & isMinObjetive"
                  type="button"
                  @click.prevent="add()"
...
export default {
  ...
  data: () => ({
    ...
    titlecolum: "Objetivos del Proyecto Col",
    titleBtn: "Objetivos",
  ...
  computed: {
    dataSource: function () {
      ...
      return DxStore({
        key: ["id"],
        endPoint: `proposals/${root.propuesta.id}/objectives`,
  ...
  methods: {
      ...
      await root.getSinData({
        ...
        cb: function (results) {
          ...
          if (data.data.length > 0) {
            root.baseObjGen = data.data[0];
            root.baseObj.parent_id = root.baseObjGen.id;
            root.baseEnt.parent_id = root.baseObjGen.id;
            root.titleBtn = "Crear Objetivo Específico";
            root.isMinObjetive = results.data.length < 6 ? true : false;
            console.warn("resultado", root.isMinObjetive);
            console.warn("total conunt ", results.data.length);
          } else {
            root.titleBtn = "Crear Objetivo General";
            // root.baseObj.parent_id=0;
            // root.baseEnt.parent_id=0;
          }
        },
      });
    },
    ...
    save() {
      ...
      if (result.isValid) {
        ...
        let dto = {
          newFormat: true,
          rute2: "proposals/" + root.propuesta.id + "/objectives",
          stringEP: "objectives",
          mod: obj.id,
          objectSend: JSON.parse(`{ "objective": ` + JSON.stringify(obj) + "}"),
          cb: function (item) {
    ...
    active(data, state) {
      ...
      let msg = `¿Realmente desea ${a} el objetivo:<br> <span class='text-sb'>"${data.data.description}"</span>?`;
      this.$confirm(msg, function (si_no) {
        ...
        if (si_no) {
          ...
          var dto = {
            newFormat: true,
            url: `objectives/${data.data.id}`,
            data: JSON.parse(`{ "objective" :` + active + "}"),
            cb: function (result) {
              console.log("Result", result);
              root.grid.refresh();
              root.loaderHide();
