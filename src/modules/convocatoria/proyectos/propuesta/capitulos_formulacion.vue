										...
										<div class="row pb-3 mb-2 bb">
											<div class="col">
												<div class="col d-flex justify-content-between align-items-end">
													<div>
														<span class="title d-block"><i class="icon-pencil6"></i> Formulación de Propuesta {{ propuesta.title }}</span>
														<span class="title2 d-block mt-2">Metodología de Trabajo</span>
													</div>
													<div class="sub-title text-nowrap ml-4"><i class="icon-page-break"></i>&nbsp;Plantilla</div>
												</div>
											</div>
										</div>
										<div class="row">
											...
											<div class="col" v-else>
												<div class="row mt-5">
													<div class="col mt-5 title2">
														<i class="icon-point-left mr-2 d-inline-block"></i>Haga clic en algún ítem para editarlo o arrastre y suelte para modificar su
														orden.
													</div>
												</div>
												<div class="row">
			...
			<div class="col">
				<div class="card">
					<div class="card-body">
						...
						<!-- <hr class="sep mb-0" /> -->
						<span class="font-weight-semibold">propuesta:</span>
						<pre>{{ JSON.stringify(propuesta, null, 3) }}</pre>
					</div>
				</div>
			</div>
		</div>
...
let $ = window.jQuery,
	root = null;
import { DxButton, DxDateBox, DxFileUploader, DxSelectBox, DxSwitch, DxTagBox, DxTextArea, DxTextBox, DxValidationGroup, DxNumberBox } from "devextreme-vue";
...
export default {
	...
	created: async function() {
		...
		// let uId = root.$route.params.convocatoriaId;
		let uId = root.propuesta.id;
		// root.item = await root.getItem(uId);
		console.log("root.item =>", root.item);
		document.title += ` Propuesta ${root.propuesta.title}`;
		...
		setTimeout(async function() {
			...
			let caps = await root.capitulos({ context: "proposals", id: uId });
			//let caps = [];
			console.log("CAPS EN BD =>", caps);
			// Determina si ya se crearon capítulos
			if (caps.length > 0) {
				console.warn("caps.length > 0");
				console.warn(caps);
	...
	async mounted() {
		...
		// root.proposal.proposal_status_id = root.estadoProyecto.find(
		//     (element) => element.st_name == "Borrador"
		// ).id;

		if (root.propuesta != null && typeof root.propuesta != undefined) {
			...
			await root.get({
				...
				cb: function(results) {
					...
					root.proposal.research_focus_ids = objeto.research_focuses.map((lineasinv) => lineasinv.research_focus_id);
					root.loaderHide();
				},
			});
		}
	...
	data: () => ({
		...
		//seccion del componente de plantilla
	...
	methods: {
		...
		...mapActions("core/propuesta_capitulos", {
			getTemplate: "getTemplate",
			capitulos: "getAll",
			crearCapitulos: "createAll",
			guardarCapitulos: "updateAll",
			guardarCapitulo: "update",
		}),
		// ...mapActions("convocatoria", ["getItem", "getTemplate", "getCriteria", "getFinancingItems", "getActivities", "getDocuments", "getItems"]),
		//  ...mapActions("convocatoria", ["getTemplate"]),
		...
		saveDoc: async () => {
			console.warn("results => root.document_items=>", root.document_items);
			root.loaderShow("Guardando Plantilla", "#panel-documentos .card-body");
			root.document_items.udpated_by = root.user_id;

			root.document_items.forEach((item, idx) => {
				if (item.updated_by == null) item.updated_by = root.user_id;
			});
        ...
        */
		...
		setContents: (items) => {
            ...
            items.forEach((item) => {
				...
				if (item.ch_order == 1) {
					// Dirigida a
					item.ch_description = "call_directed_towards"; //root.item.call_directed_towards;
				} else if (item.ch_order == 3) {
					// Objetivos
					item.ch_description = "call_objective"; //root.item.call_objective;
