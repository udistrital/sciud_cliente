...
Guia Practica Clinica = Titulo botones 
Guia Practica Clinica = Titulo principal 
Guia_Practica=titulo abreviado
pabelConcepGuia_Practica=nombredepaneles
clinical_practice_guidelines = endpoint enlace
clinical_practice_guideline=endpoindt especifico endpoinds tutas generales update
*\
<template>
	<div class="col mt-3 pl-1 pr-1" id="pabelConcepGuia_Practica">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Guia Practica Clinica</span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" v-if="editMode"  title="Nuevo Guia Practica Clinica.." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-database-add"></i></b> Nueva Guia Practica Clinica
								</button>
							</span>
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Guia Practica Clinica.." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Guia Practica Clinica
								</button>
							</span>
						</div>
					</div>
        ...
        <Documentos id="pabelConcepGuia_Practica-documentos" end-point="clinical_practice_guidelines" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="pabelConcepGuia_Practica-participantes" end-point="clinical_practice_guidelines" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Guia Practica Clinica</span>
						</div>
			...
			<div class="col">
				<div class="p-0">
					...
					>
                        ...
                        <DxColumn data-field='date_of_publication'  caption='Fecha Publicacion' data-type='string' alignment='center' :visible='true' :allow-grouping='true' /> 
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								<a v-if="data.data.active" title="Desactivar Guia_Practica..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Guia_Practica..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("guia_practica_categoria");
		root.tiposDocumento = root.subtypesByType("guia_practica_documento");
	},
	mounted() {
		console.log("root.tipos", root.tipos);
		root.panelData = $("#pabelConcepGuia_Practica .data");
		root.panelGrid = $("#pabelConcepGuia_Practica .grid");
		root.panelCmds = $("#pabelConcepGuia_Practica .cmds");
		root.panelCmdBack = $("#pabelConcepGuia_Practica .cmds-back");
		root.panelDocs = $("#pabelConcepGuia_Practica-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#pabelConcepGuia_Practica .grid";
	},
	computed: {
		...
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${root.group.id}/clinical_practice_guidelines`,
	...
	methods: {
		...
		participantes(data) {
			...
			$("#pabelConcepGuia_Practica .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.title}`);
			root.panelParticipantes = $("#pabelConcepGuia_Practica-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#pabelConcepGuia_Practica-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#pabelConcepGuia_Practica-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#pabelConcepGuia_Practica .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo;  ${data.row.data.title}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#pabelConcepGuia_Practica-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#pabelConcepGuia_Practica-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#pabelConcepGuia_Practica .item-title").html("");
		...
		save() {
			...
			if (result.isValid) {
				...
				let dto = {
					...
					stringEP: "clinical_practice_guidelines",
					mod: obj.id,
					objectSend: { clinical_practice_guideline: obj },
					cb: function(item) {
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					...
					var dto = {
                        //updated_by: 1,
						//url: `research_units/${root.group.id}/clinical_practice_guidelines/${data.data.id}/active`,
                        newFormat:true,
                        url: `clinical_practice_guidelines/${data.data.id}`,
						data: {
