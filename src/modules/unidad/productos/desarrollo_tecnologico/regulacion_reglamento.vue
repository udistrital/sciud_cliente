					...
					<div class="page-header header-elements-md-inline mb-2">
						...
						<div class="header-elements">
							...
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Reglamentos .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Reglamento
								</button>
							</span>
						</div>
					</div>
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Reglamentos </span>
						</div>
					...
					>
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								<a v-if="data.data.active" title="Desactivar Reglamentos..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Reglamentos..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("regulacion_reglamento_categoria");
		root.tiposDocumento = root.subtypesByType("regulacion_reglamento_documento");
	},
	mounted() {
		console.log("root.tipos", root.tipos);
		root.panelData = $("#panelReglamentos .data");
		root.panelGrid = $("#panelReglamentos .grid");
		root.panelCmds = $("#panelReglamentos .cmds");
		root.panelCmdBack = $("#panelReglamentos .cmds-back");
		root.panelDocs = $("#panelReglamentos-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#panelReglamentos .grid";
	},
	computed: {
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelReglamentos .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.title}`);
			root.panelParticipantes = $("#panelReglamentos-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelReglamentos-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelReglamentos-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelReglamentos .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo;  ${data.row.data.title}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelReglamentos-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelReglamentos-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelReglamentos .item-title").html("");
