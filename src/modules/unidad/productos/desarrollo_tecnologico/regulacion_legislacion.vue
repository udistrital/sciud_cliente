...
Legislaciones  = Titulo botones 
Legislaciones  = Titulo principal 
Legislaciones=titulo abreviado
panelLegislaciones=nombredepaneles
regulations = regulation enlace
regulation=endpoindt especifico endpoinds tutas generales update
*\
<template>
	<div class="col mt-3 pl-1 pr-1" id="panelLegislaciones">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Legislaciones  </span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" v-if="editMode"  title="Nueva Legislacion .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-database-add"></i></b> Nuevo Legislacion
								</button>
							</span>
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Legislaciones .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Legislacion
								</button>
							</span>
						</div>
					</div>
        ...
        <Documentos id="panelLegislaciones-documentos" end-point="regulations" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="panelLegislaciones-participantes" end-point="regulations" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Legislaciones </span>
						</div>
			...
			<div class="col">
				<div class="p-0">
					...
					>
                        ...
                        <DxColumn data-field='regulation_type_name'  caption='Tipo de Legislacion' data-type='string' alignment='center' :visible='true' :allow-grouping='true' /> 
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								<a v-if="data.data.active" title="Desactivar Legislaciones..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Legislaciones..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("regulacion_legislacion_categoria");
		root.tiposDocumento = root.subtypesByType("regulacion_reglamento_documento");
	},
	mounted() {
		console.log("root.tipos", root.tipos);
		root.panelData = $("#panelLegislaciones .data");
		root.panelGrid = $("#panelLegislaciones .grid");
		root.panelCmds = $("#panelLegislaciones .cmds");
		root.panelCmdBack = $("#panelLegislaciones .cmds-back");
		root.panelDocs = $("#panelLegislaciones-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#panelLegislaciones .grid";
	},
	computed: {
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelLegislaciones .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.title}`);
			root.panelParticipantes = $("#panelLegislaciones-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelLegislaciones-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelLegislaciones-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelLegislaciones .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo;  ${data.row.data.title}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelLegislaciones-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelLegislaciones-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelLegislaciones .item-title").html("");
