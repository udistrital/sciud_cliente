<template>
	<div class="col mt-3 pl-1 pr-1" id="panelPlantaind">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">{{ title }}.{{ tipo }}</span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" title="nueva Planta.." class="btn btn-main btn-labeled btn-labeled-left ">
		...
		<Documentos id="panelPlantaind-documentos" end-point="plant_ind_prototypes" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="panelPlantaind-participantes" end-point="plant_ind_prototypes" :product="baseObj" :group="group" ref="participantes" :parent="this" />
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("planta_industrial_categoria");
		root.tiposDocumento = root.subtypesByType("planta_industrial_documento");
	},
	mounted() {
		//root.getConvocatorias();
		// root.tipos = root.subtypesByType(12);
		// root.subtipos = root.subtypesByType(35);
		//root.tipos = root.subtypesByType("planta_industrial");
		console.log("root.tipos", root.tipos);
		root.panelData = $("#panelPlantaind .data");
		root.panelGrid = $("#panelPlantaind .grid");
		root.panelCmds = $("#panelPlantaind .cmds");
		root.panelCmdBack = $("#panelPlantaind .cmds-back");
		root.panelDocs = $("#panelPlantaind-documentos");
		root.loaderMessage = "Cargando elementos";
		root.loaderElement = "#panelPlantaind .grid";
	},
	computed: {
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelPlantaind .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.title}`);
			root.panelParticipantes = $("#panelPlantaind-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelPlantaind-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelPlantaind-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelPlantaind .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo; ${data.row.data.title}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelPlantaind-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelPlantaind-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelPlantaind .item-title").html("");
