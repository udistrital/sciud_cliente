...
Innovación en procedimiento y servicio  = Titulo botones 
Innovación en procedimiento y servicio  = Titulo principal 
Innovacion_generada=titulo abreviado
panelInnovacion_procedimiento=nombredepaneles
innovations = innovation enlace
innovation=endpoindt especifico endpoinds tutas generales update
*\
<template>
	<div class="col mt-3 pl-1 pr-1" id="panelInnovacion_procedimiento">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Innovación en procedimiento y servicio  </span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" v-if="editMode"  title="Nueva Innovación en procedimiento y servicio .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-database-add"></i></b> Nueva Innovación en procedimiento
								</button>
							</span>
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Innovación en procedimiento y servicio .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Innovación en procedimiento
								</button>
							</span>
						</div>
					</div>
        ...
        <Documentos id="panelInnovacion_procedimiento-documentos" end-point="innovations" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="panelInnovacion_procedimiento-participantes" end-point="innovations" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Innovación en procedimiento y servicio </span>
						</div>
...
<div class="col-md-4">
	...
	<label>Nombre de la Innovación: </label>
	<DxTextBox placeholder="Nombre de la Innovación" class="form-control" :value.sync="baseObj.name">
	<DxValidator>
		<DxRequiredRule />
	</DxValidator>
	</DxTextBox>
...
<div class="col-md-4">
	...
	<DxTextBox placeholder="Nombre de la Compañía" class="form-control" :value.sync="baseObj.company_name">
	<DxValidator>
	</DxValidator>
	</DxTextBox>
	</div>
			...
			<div class="col">
				<div class="p-0">
					...
					>
						...
						<DxColumn data-field='nit'  caption='NIT' data-type='string' alignment='center' :visible='true' :allow-grouping='false' /> 
						<DxColumn data-field='company_name'  caption='Nombre Compañía' data-type='string' alignment='center' :visible='true' :allow-grouping='false' /> 
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								<a v-if="data.data.active" title="Desactivar Innovacion_generada..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Innovacion_generada..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("innovacion_procedimiento_categoria");
		root.tiposDocumento = root.subtypesByType("innovacion_procedimiento_documento");
	},
	mounted() {
		console.log("root.tipos", root.tipos);
		root.panelData = $("#panelInnovacion_procedimiento .data");
		root.panelGrid = $("#panelInnovacion_procedimiento .grid");
		root.panelCmds = $("#panelInnovacion_procedimiento .cmds");
		root.panelCmdBack = $("#panelInnovacion_procedimiento .cmds-back");
		root.panelDocs = $("#panelInnovacion_procedimiento-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#panelInnovacion_procedimiento .grid";
	},
	computed: {
		...
		dataSource: function() {
			...
			return DxStore({
				...
				endPoint: `research_units/${root.group.id}/innovations`,
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelInnovacion_procedimiento .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.name}`);
			root.panelParticipantes = $("#panelInnovacion_procedimiento-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelInnovacion_procedimiento-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelInnovacion_procedimiento-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelInnovacion_procedimiento .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo;  ${data.row.data.name}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelInnovacion_procedimiento-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelInnovacion_procedimiento-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelInnovacion_procedimiento .item-title").html("");
		...
		save() {
			...
			if (result.isValid) {
				...
				let dto = {
					...
					stringEP: "innovations",
					mod: obj.id,
					objectSend: { innovation: obj },
					cb: function(item) {
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					...
					var dto = {
                        //updated_by: 1,
						//url: `research_units/${root.group.id}/innovations/${data.data.id}/active`,
                        newFormat:true,
                        url: `innovations/${data.data.id}`,
						data: {
							innovation: {
								active: state,
