\* ind_dsg_registration_title=titulo para activar o desactivar Diseño Industrial= Titulo botones Diseño Industrial=Titulo principal Diseño=titulo abreviado
paneldiseno=nombredepaneles industrial_design=endpoindt especifico endpoinds tutas generales *\
<template>
	<div class="col mt-3 pl-1 pr-1" id="paneldiseno">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Diseño Industrial</span>
						...
						<div class="header-elements">
							...
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Diseño Industrial.." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Diseño Industrial
								</button>
							</span>
						</div>
					</div>
		...
		<Documentos id="paneldiseno-documentos" end-point="industrial_designs" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="paneldiseno-participantes" end-point="industrial_designs" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Diseño Industrial</span>
						</div>
							...
							<div class="row">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<DxTextBox placeholder="Título de registro" class="form-control" :value.sync="baseObj.ind_dsg_registration_title">
											<DxValidator>
												<DxRequiredRule />
											</DxValidator>
										</DxTextBox>
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<DxTextBox
											...
											class="form-control"
											:value.sync="baseObj.ind_dsg_industrial_publication_gazette"
										>
											<DxValidator> </DxValidator>
										</DxTextBox>
									</div>
			...
			<div class="col">
				<div class="p-0">
					...
					>
						...
						<DxColumn
							data-field="ind_dsg_date_of_obtaining"
							caption="Adquisicion"
							data-type="string"
							alignment="left"
							:visible="true"
						...
						/>
						<DxColumn
							data-field="ind_dsg_industrial_publication_gazette"
							caption="N° Revista"
							data-type="string"
							alignment="left"
							:visible="true"
						...
						/>
						<DxColumn
							data-field="ind_dsg_registration_number"
							caption="N° Registro"
							data-type="string"
							alignment="left"
							:visible="true"
							:allow-grouping="false"
						/>
						<DxColumn data-field="observation" caption="Observaciones" data-type="string" alignment="left" :visible="true" :allow-grouping="false" />
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								<a v-if="data.data.active" title="Desactivar Diseño..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Diseño..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("diseno_industrial_categoria");
		root.tiposDocumento = root.subtypesByType("diseno_industrial_documento");
	},
	mounted() {
		//root.getConvocatorias();
		//root.tipos = root.subtypesByType(5);
		...
		root.panelData = $("#paneldiseno .data");
		root.panelGrid = $("#paneldiseno .grid");
		root.panelCmds = $("#paneldiseno .cmds");
		root.panelCmdBack = $("#paneldiseno .cmds-back");
		root.panelDocs = $("#paneldiseno-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#paneldiseno .grid";
	},
	computed: {
		...
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${root.group.id}/industrial_designs`,
	...
	methods: {
		...
		participantes(data) {
			...
			root.panelGrid.fadeOut(function(params) {
				...
				$("#paneldiseno-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#paneldiseno-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#paneldiseno-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
		...
		save() {
			...
			if (result.isValid) {
				...
				let dto = {
					...
					stringEP: "industrial_designs",
					mod: obj.id,
					objectSend: { industrial_design: obj },
					cb: function(item) {
		...
		active(data, state) {
			...
			let msg = `¿Realmente desea ${a} <span class='text-sb'>"${data.data.ind_dsg_registration_title}"</span>?`;
			this.$confirm(msg, function(si_no) {
				console.log("result", si_no);
				if (si_no) {
					root.loaderShow(`${am}`, root.panelGrid);
					var dto = {
						url: `research_units/${root.group.id}/industrial_designs/${data.data.id}/active`,
						data: {
							industrial_design: {
								active: state,
								updated_by: 1,
