...
Innovación generada en la gestión empresarial  = Titulo botones 
Innovación generada en la gestión empresarial  = Titulo principal 
Innovacion_generada=titulo abreviado
panelInnovacion_generada=nombredepaneles
innovations = innovation enlace
innovation=endpoindt especifico endpoinds tutas generales update
*\
<template>
	<div class="col mt-3 pl-1 pr-1" id="panelInnovacion_generada">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Innovación generada en la gestión empresarial  </span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" v-if="editMode"  title="Nueva Innovación generada en la gestión empresarial .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-database-add"></i></b> Nueva Innovación Generada
								</button>
							</span>
							<span class="cmds-back slide">
								<button type="button" @click.prevent="retorno()" title="Volver al Innovación generada en la gestión empresarial .." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-arrow-left"></i></b> Volver A Innovación Generada
								</button>
							</span>
						</div>
					</div>
        ...
        <Documentos id="panelInnovacion_generada-documentos" end-point="innovations" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="panelInnovacion_generada-participantes" end-point="innovations" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Innovación generada en la gestión empresarial </span>
						</div>
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("innovacion_generada_categoria");
		root.tiposDocumento = root.subtypesByType("innovacion_generada_documento");
	},
	mounted() {
		console.log("root.tipos", root.tipos);
		root.panelData = $("#panelInnovacion_generada .data");
		root.panelGrid = $("#panelInnovacion_generada .grid");
		root.panelCmds = $("#panelInnovacion_generada .cmds");
		root.panelCmdBack = $("#panelInnovacion_generada .cmds-back");
		root.panelDocs = $("#panelInnovacion_generada-documentos");
		root.loaderMessage = "Cargando Elementos";
		root.loaderElement = "#panelInnovacion_generada .grid";
	},
	computed: {
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelInnovacion_generada .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.name}`);
			root.panelParticipantes = $("#panelInnovacion_generada-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelInnovacion_generada-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelInnovacion_generada-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelInnovacion_generada .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo;  ${data.row.data.name}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelInnovacion_generada-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelInnovacion_generada-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelInnovacion_generada .item-title").html("");
