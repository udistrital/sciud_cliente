/* Elementos a remplazar: Titulo1 = Titulos Principales de la aplicacion en singular una palabra titulo2 = Son para los botones de nuevo elemento titulo3 =
titulo abrebiado de la del contenido a mostrar titulo4 = nombre del archivo store al cual esta configurado axios research_units= ruta principal del modulo en el
endPoint objanson = objeto json al cual se enviaran los datos de formulario */

<template>
	<div class="col mt-3" id="panel-Titulo1" v-if="editMode">
		<div class="row data slide">
			<div class="col">
				<div class="card">
					<div class="card-header main">
						<i class="icon-pencil3 mr-1 color-main-600"></i>
						<span class="font-weight-semibold">{{ editData ? "Editar" : "Crear" }} Titulo1</span>
					</div>
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<DxTextBox placeholder="Titulo Titulo1" class="form-control" :value.sync="objanson.title">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxDateBox
										...
										:value.sync="objanson.publication_date"
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:value.sync="objanson.category_id"
										class="form-control"
							...
							<div class="col-md-6"><Geo :lockElement="loaderElement" :syncObject="objanson" /></div>
							...
							<div class="col-md-2">
								<div class="form-group">
									<label>Activo: </label>
									<DxSwitch :value.sync="objanson.Active" switched-on-text="SI" switched-off-text="NO" />
								</div>
							</div>
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<DxTextArea :height="100" :max-length="400" :value.sync="objanson.observation" placeholder="Observaciones" class="form-control">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextArea>
				...
				<div class="p-0">
					<div class="page-header header-elements-md-inline">
						...
						<div class="header-elements">
							<button type="button" @click.prevent="edit()" title="Titulo2.." class="btn btn-main btn-labeled btn-labeled-left " id="btn-add">
								<b><i class="icon-database-add"></i></b>Titulo2
							</button>
						</div>
					</div>
					...
					>
						...
						<DxSummary>
							<DxGroupItem summary-type="count" column="group_type_name" display-format="{0} titulo1" />
						</DxSummary>
						<DxPager
							:show-info="true"
							...
							info-text="Página {0} de {1} ({2} titulo1)"
						/>
						...
						<template #tpl="{ data }">
							<span class="cmds">
								<a title="Editar titulo1..." class="cmd-item color-main-600" @click.prevent="edit(data.data)" href="#">
								...
								-->
								<a v-if="data.data.active" title="Desactivar Titulo1..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Titulo1..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	mounted() {
		// console.clear();
		root = this;
		// root.loaderElement = "#panel-Titulo1";
		// this.loaderElement = ".card-body";
		this.loaderElement = "#panel-unidades .card-body";
		this.loaderMessage = "Cargando titulo3";
		root.panelData = $("#panel-Titulo1 .data");
		root.panelGrid = $("#panel-Titulo1 .grid");
		// console.clear();
	...
	computed: {
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${this.group.id}/objansons`,
				// endPoint: `research_units/objansons`,
	...
	methods: {
		...
		...mapActions("Titulo4", { objansonActive: "active" }),
		...mapActions("titulo4", ["all", "saveChange", "updateChange"]),
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					...
					root.loadShow();
					var dto = {
						url: `research_units/${root.group.id}/objansons/${data.data.id}/active`,
						objanson: {
							objanson: {
								active: state,
								updated_by: 1,
