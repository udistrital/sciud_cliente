\* data.data.journal_name=titulo para activar o desactivar Eventos artísticos, de arquitectura o de diseño= Titulo botones Eventos artísticos, de arquitectura o
de diseño=Titulo principal Registro=titulo abreviado panelRegistro=nombredepaneles event=endpoindt especifico events rutas generales *\
<template>
	<div class="col mt-3 pl-1 pr-1" id="panelRegistro">
		<div class="row">
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline mb-2">
						<div class="page-title p-0 m-0">
							<h1>
								...
								<span class="font-weight-semibold">Eventos artísticos, de arquitectura o de diseño</span>
								<span class="item-title">&nbsp;</span>
							</h1>
						</div>
						<div class="header-elements">
							<span class="cmds">
								<button type="button" @click.prevent="add()" v-if="editMode"  title="Nuevo Eventos.." class="btn btn-main btn-labeled btn-labeled-left ">
									<b><i class="icon-database-add"></i></b> Nuevo Evento
								</button>
							</span>
							<span class="cmds-back slide">
								<button
									...
									title="Volver al Eventos artísticos, de arquitectura o de diseño.."
									class="btn btn-main btn-labeled btn-labeled-left "
								>
									<b><i class="icon-arrow-left"></i></b> Volver s Eventos
								</button>
							</span>
						</div>
					</div>
		...
		<Documentos id="panelRegistro-documentos" end-point="events" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="panelRegistro-participantes" end-point="events" :product="baseObj" :group="group" ref="participantes" :parent="this" />
				...
				<div class="col">
					<div class="card">
						<div class="card-header main">
							...
							<span class="font-weight-semibold">{{ mode == "edit" ? "Editar" : "Crear" }} Eventos artísticos, de arquitectura o de diseño</span>
						</div>
...
export default {
	...
	created() {
		...
		root.subtipos = root.subtypesByType("evento_cultural_categoria");
		root.tiposDocumento = root.subtypesByType("evento_cultural_documento");
	},
	mounted() {
		// root.getConvocatorias();
		...
		root.panelData = $("#panelRegistro .data");
		root.panelGrid = $("#panelRegistro .grid");
		root.panelCmds = $("#panelRegistro .cmds");
		root.panelCmdBack = $("#panelRegistro .cmds-back");
		root.panelDocs = $("#panelRegistro-documentos");
		root.loaderMessage = "Cargando elementos";
		root.loaderElement = "#panelRegistro .grid";
	},
	computed: {
	...
	methods: {
		...
		participantes(data) {
			...
			$("#panelRegistro .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.eve_name}`);
			root.panelParticipantes = $("#panelRegistro-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#panelRegistro-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelRegistro-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#panelRegistro .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo; ${data.row.data.eve_name}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#panelRegistro-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#panelRegistro-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#panelRegistro .item-title").html("");
