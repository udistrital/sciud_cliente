/* Elementos a remplazar: Pregrado = Titulos Principales de la aplicacion en singular una palabra Nuevo Pregrado = Son para los botones de nuevo elemento
Pregrado = titulo abrebiado de la del contenido a mostrar pregradojs = nombre del archivo store al cual esta configurado axios research_units= ruta principal
del modulo en el endPoint undergraduate = objeto json al cual se enviaran los datos de formulario */

<template>
	<div class="col mt-3" id="panel-Pregrado" v-if="editMode">
		<div class="row data slide">
			<div class="col">
				<div class="card">
					<div class="card-header main">
						<i class="icon-pencil3 mr-1 color-main-600"></i>
						<span class="font-weight-semibold">{{ editData ? "Editar" : "Crear" }} Pregrado</span>
					</div>
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<DxTextBox placeholder="Titulo Pregrado" class="form-control" :value.sync="undergraduate.title">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxDateBox
										...
										:value.sync="undergraduate.publication_date"
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:value.sync="undergraduate.category_id"
										class="form-control"
							...
							<div class="col-md-6"><Geo :lockElement="loaderElement" :syncObject="undergraduate" /></div>
							...
							<div class="col-md-2">
								<div class="form-group">
									<label>Activo: </label>
									<DxSwitch :value.sync="undergraduate.Active" switched-on-text="SI" switched-off-text="NO" />
								</div>
							</div>
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<DxTextArea :height="100" :max-length="400" :value.sync="undergraduate.observation" placeholder="Observaciones" class="form-control">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextArea>
				...
				<div class="p-0">
					<div class="page-header header-elements-md-inline">
						...
						<div class="header-elements">
							<button type="button" @click.prevent="edit()" title="Nuevo Pregrado.." class="btn btn-main btn-labeled btn-labeled-left " id="btn-add">
								<b><i class="icon-database-add"></i></b>Nuevo Pregrado
							</button>
						</div>
					</div>
					...
					>
						...
						<DxSummary>
							<DxGroupItem summary-type="count" column="group_type_name" display-format="{0} Pregrado" />
						</DxSummary>
						<DxPager
							:show-info="true"
							...
							info-text="Página {0} de {1} ({2} Pregrado)"
						/>
						...
						<template #tpl="{ data }">
							<span class="cmds">
								<a title="Editar Pregrado..." class="cmd-item color-main-600" @click.prevent="edit(data.data)" href="#">
								...
								-->
								<a v-if="data.data.active" title="Desactivar Pregrado..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Pregrado..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
		...
		<div class="card mt-3" v-if="debug">
			<div class="card-body">
				{{ JSON.stringify(undergraduate, null, "\t") }}
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	mounted() {
		// console.clear();
		root = this;
		// root.loaderElement = "#panel-Pregrado";
		// this.loaderElement = ".card-body";
		this.loaderElement = "#panel-unidades .card-body";
		this.loaderMessage = "Cargando Pregrado";
		root.panelData = $("#panel-Pregrado .data");
		root.panelGrid = $("#panel-Pregrado .grid");
		// console.clear();
	...
	computed: {
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${this.group.id}/undergraduates`,
				// endPoint: `research_units/undergraduates`,
	...
	methods: {
		...
		...mapActions("pregradojs", { undergraduateActive: "active" }),
		...mapActions("pregradojs", ["all", "saveChange", "updateChange"]),

		save() {
			let args = {
				unidadId: root.group.id,
				data: root.undergraduate,
			};
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					...
					root.loadShow();
					var dto = {
						url: `research_units/${root.group.id}/undergraduates/${data.data.id}/active`,
						undergraduate: {
							undergraduate: {
								active: state,
								updated_by: 1,
