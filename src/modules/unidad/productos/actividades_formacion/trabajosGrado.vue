\* data.data.dw_title=titulo para activar o desactivar {{ title }}= Titulo botones {{ title }}=Titulo principal Trabajos=titulo abreviado
paneltrabajos=nombredepaneles degree_work=endpoindt especifico degree_works rutas generales *\
<template>
	<div class="col mt-3 pl-1 pr-1" id="paneltrabajos">
		<div class="row">
			<div class="col">
				<div class="p-0">
		...
		<Documentos id="paneltrabajos-documentos" end-point="degree_works" :main-obj="baseObj" :parent="this" :tipos="tiposDocumento" />
		<Participantes id="paneltrabajos-participantes" end-point="degree_works" :product="baseObj" :group="group" ref="participantes" :parent="this" />
			...
			<div class="col">
				<div class="p-0">
					...
					>
						...
						<DxColumn
							data-field="colciencias_call_id"
...
export default {
	...
	created() {
		...
		// root.baseObj.dw_type_id=root.id_trabajo;
	...
	mounted() {
		...
		root.panelData = $("#paneltrabajos .data");
		root.panelGrid = $("#paneltrabajos .grid");
		root.panelCmds = $("#paneltrabajos .cmds");
		root.panelCmdBack = $("#paneltrabajos .cmds-back");
		root.panelDocs = $("#paneltrabajos-documentos");
		root.loaderMessage = "Cargando elementos";
		root.loaderElement = "#paneltrabajos .grid";
	},
	computed: {
		...
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				// ids: ["dw_type_id=1"],
				stringParam: "dw_type_id=" + root.id_trabajo,
	...
	methods: {
		...
		participantes(data) {
			...
			$("#paneltrabajos .item-title").html(`<span class="font-weight-semibold"> &raquo; Participantes</span> &raquo; ${data.row.data.title}`);
			root.panelParticipantes = $("#paneltrabajos-participantes");
			console.log("root.panelParticipantes", root.panelParticipantes.length);
			$("#paneltrabajos-documentos").hide();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#paneltrabajos-participantes .grid").fadeIn();
		...
		documentos(data) {
			...
			console.log("rd", rd);
			root.baseObj = rd;
			$("#paneltrabajos .item-title").html(`<span class="font-weight-semibold"> &raquo; Documentos</span> &raquo; ${data.row.data.title}`);
			root.panelCmds.fadeOut();
			root.panelGrid.fadeOut(function(params) {
				root.panelCmdBack.fadeIn();
				$("#paneltrabajos-documentos").fadeIn(function(params) {});
			});
		},
		...
		retorno() {
			...
			if (root.section == "participantes") {
				...
				$("#paneltrabajos-documentos").fadeOut(function(params) {
					root.panelCmds.fadeIn();
					root.panelGrid.fadeIn(function(params) {});
				});
			}
			$("#paneltrabajos .item-title").html("");
