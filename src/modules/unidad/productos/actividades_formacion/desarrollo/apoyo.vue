/* Elementos a remplazar: Apoyo = Titulos Principales de la aplicacion en singular una palabra Nueva Creación de Programas = Son para los botones de nuevo
elemento Creación de Programas = titulo abrebiado de la del contenido a mostrar creación deProgramasjs = nombre del archivo store al cual esta configurado axios
research_units= ruta principal del modulo en el endPoint research_training = objeto json al cual se enviaran los datos de formulario */

<template>
	<div class="col mt-3" id="panel-Apoyo" v-if="editMode">
		<div class="row data slide">
			<div class="col">
				<div class="card">
					<div class="card-header main">
						<i class="icon-pencil3 mr-1 color-main-600"></i>
						<span class="font-weight-semibold">{{ editData ? "Editar" : "Crear" }} Apoyo</span>
					</div>
					<div class="card-body mb-0 pb-0 pt-2">
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									<label>Nombre programa o curso: </label>
									<DxTextBox placeholder="Nombre programa o curso" class="form-control" :value.sync="research_training.course_program_name">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label>Nombre programa donde se imparte: </label>
									<DxTextBox placeholder="Nombre programa donde se imparte" class="form-control" :value.sync="research_training.program_which_is_taught">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxTextBox placeholder="Institución universitaria" class="form-control" :value.sync="research_training.university_institution">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxTextBox placeholder="Facultad" class="form-control" :value.sync="research_training.faculty">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxTextBox placeholder="Departamento" class="form-control" :value.sync="research_training.department">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label>Identificación Acto Administrativo: </label>
									<DxTextBox
										placeholder="Identificación Acto Administrativo"
										class="form-control"
										:value.sync="research_training.identification_administrative_act"
									>
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label>Número Acto Administrativo: </label>
									<DxTextBox placeholder="Número Acto Administrativo" class="form-control" :value.sync="research_training.number_administrative_act">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxDateBox
										class="form-control"
										name="date"
										:value.sync="research_training.date"
										id="date"
							...
							<div class="col-md-2">
								<div class="form-group">
									<label>Activo: </label>
									<DxSwitch :value.sync="research_training.active" switched-on-text="SI" switched-off-text="NO" />
								</div>
							</div>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:value.sync="research_training.category_id"
										class="form-control"
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:search-enabled="false"
										placeholder="Seleccione..."
										:value.sync="research_training.colciencias_call_id"
										class="form-control"
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<DxTextArea :height="100" :max-length="400" :value.sync="research_training.observation" placeholder="Observaciones" class="form-control">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextArea>
			...
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline">
						...
						<div class="header-elements">
							<button
								type="button"
								@click.prevent="edit()"
								title="Nueva Creación de Programas.."
								class="btn btn-main btn-labeled btn-labeled-left "
								id="btn-add"
							>
								<b><i class="icon-database-add"></i></b>Nueva Creación de Programas
							</button>
						</div>
					</div>
					...
					>
						...
						<DxSummary>
							<DxGroupItem summary-type="count" column="group_type_name" display-format="{0} Apoyo" />
						</DxSummary>
						<DxPager
							:show-info="true"
							...
							info-text="Página {0} de {1} ({2} Apoyo)"
						/>
						...
						<template #tpl="{ data }">
							<span class="cmds">
								...
								-->
								<a v-if="data.data.active" title="Desactivar Apoyo..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Apoyo..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
...
export default {
	...
	data: () => ({
		...
		research_training: {
			course_program_name: null,
			program_which_is_taught: null,
			university_institution: null,
			faculty: null,
			department: null,
			identification_administrative_act: null,
	...
	mounted() {
		// console.clear();
		root = this;
		// root.loaderElement = "#panel-Apoyo";
		// this.loaderElement = ".card-body";
		this.loaderElement = "#panel-unidades .card-body";
		this.loaderMessage = "Cargando Creación de Programas";
		root.panelData = $("#panel-Apoyo .data");
		root.panelGrid = $("#panel-Apoyo .grid");
		// console.clear();
	...
	computed: {
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${root.group.id}/research_trainings`,
				// endPoint: `research_units/research_trainings`,
	...
	methods: {
		...
		...mapActions("creación deProgramasjs", { research_trainingActive: "active" }),
		...mapActions("creación deProgramasjs", ["all", "saveChange", "updateChange"]),

		save() {
			let args = {
				unidadId: root.group.id,
				data: root.research_training,
			};
			...
			if (this.editData) {
				...
				console.log(this.research_training);
				this.$alert("Datos guardados con exito!");
			}

			this.editData = false;
			this.research_training = {};
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					...
					root.loadShow();
					var dto = {
						url: `research_units/${root.group.id}/research_trainings/${data.data.id}/active`,
						research_training: {
							research_training: {
								active: state,
								updated_by: 1,
					...
					console.log("dto", dto);
					root.research_trainingActive(dto);
				}
			});
		},
