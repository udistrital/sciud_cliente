<template>
	<div class="col mt-3" id="panel-Variedades" v-if="editMode">
		<div class="row data slide">
			<div class="col">
				<div class="card">
					<div class="card-header main">
						<i class="icon-pencil3 mr-1 color-main-600"></i>
						<span class="font-weight-semibold">{{ editData ? "Editar" : "Crear" }} Variedades Vegetales</span>
					</div>
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									<label>Nombre: </label>
									<DxTextBox placeholder="Nombre" class="form-control" :value.sync="vegetable_variety.name">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextBox>
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:value.sync="vegetable_variety.category_id"
										class="form-control"
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<DxSelectBox
										...
										:search-enabled="false"
										placeholder="Seleccione..."
										:value.sync="vegetable_variety.colciencias_call_id"
										class="form-control"
										:data-source="subtipos"
										display-expr="st_name"
										Value-expr="id "
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<DxDateBox
										class="form-control"
										name="date"
										:value.sync="vegetable_variety.date"
										id="date"
							...
							<div class="col-md-2">
								<div class="form-group">
									<label>Cipo de Cliclo: </label>
									<DxSelectBox
										:grouped="false"
										:search-enabled="false"
										placeholder="Seleccione..."
										:value.sync="vegetable_variety.cycle_type_id"
										class="form-control"
										:data-source="subtipos"
										display-expr="st_name"
										Value-expr="id"
							...
							<div class="col-md-2">
								<div class="form-group">
									<label>Estado Peticion: </label>
									<DxSelectBox
										:grouped="false"
										:search-enabled="false"
										placeholder="Seleccione..."
										:value.sync="vegetable_variety.petition_status_id"
										class="form-control"
										:data-source="subtipos"
										display-expr="st_name"
										Value-expr="id"
							...
							<div class="col-md-6"><Geo :lockElement="loaderElement" :syncObject="vegetable_variety" /></div>

							<div class="col-md-2">
								<div class="form-group">
									<label>Activo: </label>
									<DxSwitch :value.sync="vegetable_variety.active" switched-on-text="SI" switched-off-text="NO" />
								</div>
							</div>
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<DxTextArea :height="100" :max-length="400" :value.sync="vegetable_variety.observation" placeholder="Observaciones" class="form-control">
										<DxValidator>
											<DxRequiredRule />
										</DxValidator>
									</DxTextArea>
			...
			<div class="col">
				<div class="p-0">
					<div class="page-header header-elements-md-inline">
						<div class="page-title d-flex">
							<h1>
								...
								<span class="font-weight-semibold">Variedades Vegetales</span>
							</h1>
						</div>
						<div class="header-elements">
							<button type="button" @click.prevent="edit()" title="Nueva Variedade..." class="btn btn-main btn-labeled btn-labeled-left " id="btn-add">
								<b><i class="icon-database-add"></i></b> Nueva Variedad
							</button>
						</div>
					</div>
					...
					>
						...
						<DxSummary>
							<DxGroupItem summary-type="count" column="group_type_name" display-format="{0} Variedades" />
						</DxSummary>
						<DxPager
							:show-info="true"
							...
							info-text="Página {0} de {1} ({2} Variedades)"
						/>
						...
						<DxColumn data-field="cycle_type_name" caption="Tipo de Clclo" data-type="string" alignment="center" :visible="true" :allow-grouping="true" />
						...
						<template #tpl="{ data }">
							<span class="cmds">
								<a title="Editar Variedades..." class="cmd-item color-main-600" @click.prevent="edit(data.data)" href="#">
								...
								<!-- 
										...
										-->
								<a v-if="data.data.active" title="Desactivar Variedades..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, false)" href="#">
									<i class="icon-database-remove"></i>
								</a>
								<a v-else title="Activar Variedades..." class="cmd-item color-main-600 mr-2" @click.prevent="active(data, true)" href="#">
		...
		<div class="card mt-3" v-if="debug">
			<div class="card-body">
				{{ JSON.stringify(vegetable_variety, null, "\t") }}
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	mounted() {
		...
		// root.loaderElement = "#panel-Variedades";
		// this.loaderElement = ".card-body";
		this.loaderElement = "#panel-unidades .card-body";
		this.loaderMessage = "Cargando Variedades";
		root.panelData = $("#panel-Variedades .data");
		root.panelGrid = $("#panel-Variedades .grid");
		// console.clear();
	...
	computed: {
		...
		dataSource: function() {
			...
			return DxStore({
				key: ["id"],
				endPoint: `research_units/${this.group.id}/vegetable_varieties`,
				// endPoint: `research_units/vegetable_varieties`,
	...
	methods: {
		...
		...mapActions("vegetal", { vegetable_varietyActive: "active" }),
		...
		active(data, state) {
			...
			this.$confirm(msg, function(si_no) {
				...
				if (si_no) {
					// root.loaderMessage = `${am} Variedades <span class='text-sb'>"${data.data.title}"</span>`;
					...
					console.log("dto", dto);
					root.vegetable_varietyActive(dto);
				}
			});
		},
