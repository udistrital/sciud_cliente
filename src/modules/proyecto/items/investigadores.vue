  ...
  <div class="col mt-3 pl-1 pr-1" :id="namePanel">
    ...
    <div class="card mt-3" v-if="debug">
			<div class="card-body">baseObj:
				<pre>{{ JSON.stringify(baseObj, null, "\t") }}</pre>
			</div>
...
export default {
  ...
  async created() {
    ...
    // setTimeout(function(){console.warn("roles=>",root.tiposParticipante);},1000)
  ...
  methods: {
    ...
    async getDataPropostal(identidad) {
			await root.getSinData({
				url:
					"proposals/by-internal-member?researcher_identification=" +identidad +
					'&filter=["call_id","=","' + root.propuesta.call_id+ '"]',
				cb: function(results) {
					let data = results;
            ...
            let msg = "El investigador con numero de identificación: "+identidad+",<br> se encuentra registrado en esta convocatoria de proyecto. <br> ¿Desea Continuar?";
