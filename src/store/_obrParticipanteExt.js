...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio participante ext", args.id);
			state.idlib = args.id
			...
			if (long <= 0) {
				api()
					.get(`research_creation_works/${args.id}/ext_participants`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
			...
			api()
				.post(`research_creation_works/${ids}/ext_participants`, args)
				.then((r) => {
					r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
			...
			api()
				.put(`research_creation_works/${ids}/ext_participants/${args.id}`,args)
				.then((r) => {
					r.data = args;
