...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			...
			if (long <= 0) {
				api()
					.get(`ip_livestock_breeds/${args.id}/int_participants`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
			...
			api()
				.post(`ip_livestock_breeds/${ids}/int_participants`, args)
				.then((r) => {
					r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
			...
			api()
				.put(`ip_livestock_breeds/${ids}/int_participants/${args.id}`,args)
				.then((r) => {
					r.data = args;
