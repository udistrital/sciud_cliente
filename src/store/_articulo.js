...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio art", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/papers`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/papers`,{paper: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/papers/${args.id}`,{paper: args})
					.then((r) => {
						r.data = args;
