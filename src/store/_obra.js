...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio obr", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/research_creation_works`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/research_creation_works`,{research_creation_work: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/research_creation_works/${args.id}`,{research_creation_work: args})
					.then((r) => {
						r.data = args;
