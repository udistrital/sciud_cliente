...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio cap", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/book_chapters`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/book_chapters`,{book_chapter: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/book_chapters/${args.id}`,{book_chapter: args})
					.then((r) => {
						r.data = args;
