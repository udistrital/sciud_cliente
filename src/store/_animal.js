...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio ani", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/new_animal_breeds`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/new_animal_breeds`,{new_animal_breed: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/new_animal_breeds/${args.id}`,{new_animal_breed: args})
					.then((r) => {
						r.data = args;
