...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio lib", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/books`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/books`,{book: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/books/${args.id}`,{book: args})
					.then((r) => {
						r.data = args;
