...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio not", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/scientific_notes`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/scientific_notes`,{scientific_note: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/scientific_notes/${args.id}`,{scientific_note: args})
					.then((r) => {
						r.data = args;
