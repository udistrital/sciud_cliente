...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio pat", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/patents`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/patents`,{patent: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/patents/${args.id}`,{patent: args})
					.then((r) => {
						r.data = args;
