...
const store = {
	...
	actions: {
		all({ commit, state }, args) {
			console.log("mio veg", args.unidadId);
			...
			if (long <= 0) {
				api()
					.get(`research_group/${args.unidadId}/vegetable_varieties`)
					.then((r) => {
		...
		saveChange({ commit, state,dispatch }, args) {
				...
				api()
					.post(`research_group/${args.unidadId}/vegetable_varieties`,{vegetable_variety: args})
					.then((r) => {
						r.data = args;
		...
		updateChange({ commit, state,dispatch }, args) {
				...
				api()
					.put(`research_group/${args.unidadId}/vegetable_varieties/${args.id}`,{vegetable_variety: args})
					.then((r) => {
						r.data = args;
